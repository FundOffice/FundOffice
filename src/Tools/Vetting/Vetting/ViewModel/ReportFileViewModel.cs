...
public partial class ReportFileViewModel : ObservableObject
{
    ...
    private async Task FillTemplateAsync()
    {
        ...
        var recommendIds = RecommendedFunds.Select(f => f.Entity.Id).ToArray();
        if (recommendIds.Length == 0)
        {
            using var db = new VettingDbContext();
            var rec = db.TemplateRecommends.FindOne(r => r.FileHash == "__global__");
            if (rec?.FundIds != null)
                recommendIds = rec.FundIds.Split(',', StringSplitOptions.RemoveEmptyEntries).Select(int.Parse).ToArray();
        }
        ...
        foreach (var jsonPath in jsonFiles)
        {
            ...
            try
            {
                ...
                Output.Add($"[{providerId}] 已生成: {outPath}");
            }
            catch (Exception ex)
            {
