...
- **资料清单/附件清单表**：不生成 operations，内容进 `files` 数组。识别特征：含"编号"+"资料"/"材料"列，或整行都是附件名称
- **嵌套表格**：每个子表格区也要覆盖

#### 不生成 operations 的表格（例外清单）

以下表格**不出现在 operations 中**，也不用 Type g 占位：

1. **附件清单/资料清单表**：列为"编号/资料/备注"或"序号/材料名称/要求"等，内容全部是需要收集的文件名 → 写入 `files` 数组
2. **勾选表格**：含 ☑是 □否 或 □是 □否 的合规/内控核查表
3. **单行标题表**：只有 1 行数据（通常是分区标题，如"公司基本情况及运营"、"团队情况"、"产品投资策略"），无实际填写内容
4. **静态参考/映射表**：内容是固定的分类标准或规范要求（如"策略分类 mapping"、"风控项目要求"），数据不需要填写
...
**产品业绩/同策略产品对比表**：列头为"产品名称"+"年化收益率"/"最大回撤"/"夏普比率"/"卡玛比率"/"波动率"/"收益回撤比"等指标的表格，使用 Type c，entity=fund，每个产品为一行，各指标列映射到对应的 FundInfo 属性（AnnualReturn、MaxDrawdown、Sharpe、Calmar、Volatility 等）。
...
1. **表格全覆盖**：从 `T[0]` 到 `T[最后一个]`，每个 table 至少出现在一个操作中。**例外（不出现在 operations 中）**：勾选表、附件清单/资料清单表、签章落款表、单行标题表、静态参考/映射表。其余没有合理理由的必须补上——无对应实体的表格用 Type g 补。
2. **索引连续**：table 不要跳号。
