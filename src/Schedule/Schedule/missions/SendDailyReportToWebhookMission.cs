...
public class SendDailyReportToWebhookMission : Mission
{
    ...
    protected override async Task<ErrorReturn> WorkOverride()
    {
        ...
        if (urls.Length == 0) return new ErrorReturn(false, "没有启用的 WebHook 地址");

        string json = ""; Exception? ex = null;
        await Application.Current.Dispatcher.InvokeAsync(async () =>
         {
             ...
             try
             {
                 ...
                 json = $"{{ \"msgtype\": \"image\", \"image\": {{\"base64\": \"{b64}\", \"md5\": \"{hash}\" }} }}";
        ...
        if (string.IsNullOrWhiteSpace(json))
            return new(false, ex?.Message ?? "未知错误");
