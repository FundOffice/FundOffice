...
public sealed class TodoViewModelRegistrationGenerator : IIncrementalGenerator
{
    ...
    private void Execute(SourceProductionContext context, ViewModelInfo[] viewModels)
    {
        ...
        // 唯一文件名，只添加一次 ✅ 修复
        context.AddSource("TodoViewModel.AutoRegister.g.cs", SourceText.From(code, Encoding.UTF8));
    }
    ...
    private string GenerateRegisterCode(List<ViewModelInfo> viewModels)
    {
        ...
        foreach (var vm in viewModels)
        {
            registerLines.AppendLine(
                $"            TodoViewModelFactory.Register<{vm.ModelFullName}>(() => new {vm.ViewModelFullName}());");
            registerLines.AppendLine(
                $"            TodoViewModelFactory.Register<{vm.ModelFullName}>((x) => new {vm.ViewModelFullName}(x as {vm.ModelFullName}));");
        }
...
public static class TodoViewModelAuto_{DateTime.Now.GetHashCode():x}
{{
    /// <summary>
